...
def basic_ks_train(model, dataloader, epoch_set, optimizer, l2_tensors, loss_function, device):
    model.train()
    model.to(device)
    ...
    for i in iterator:
        for images, labels in dataloader:
            ...
            optimizer.zero_grad()
            l2_loss = torch.sum(torch.stack([parameter ** 2 for parameter in model.parameters()]))

            l2_loss.backward()
            optimizer.step(active_masks, l2_tensors)

def get_basic_kitchen_sink_optimizer(model, lrs, l2s, epochs, device):
    ...
    for parameter in parameters:
        ...
        # L2s
        l2_tensor = torch.zeros_like(parameter, device=device)
        selections = torch.randint(0, len(l2s), l2_tensor.shape, device=device)

        for i, l2 in enumerate(l2s):
            same_selection = torch.eq(selections, i)

            l2_tensor += l2 * same_selection.float()

        assert not torch.eq(l2_tensor, 0).any()
        l2_tensors.append(l2_tensor)
    ...
    return optimizers.AdamLRMasked(parameters, lr_tensors, mask_set), lr_tensors, l2_tensors, mask_set
...
def main():
    for i in range(3, 4): # Temp
        ...
        optimizer, lr_tensors, l2_tensors, mask_set = get_basic_kitchen_sink_optimizer(model, lrs, l2s, epochs, device)
        basic_ks_train(model, dataloader, epochs, optimizer, l2_tensors, loss_function, device)
        ...
        utils.save_zip(l2_tensors, base_folder / 'l2_tensors.zip')
